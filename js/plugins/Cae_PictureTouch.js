...
/*:
 * @target MZ
 * @plugindesc v1.5 - Run events or scripts when pictures are touched/hovered.
 * @author Caethyril
 ...
 * @type boolean
 * @desc If true, 'enter' binds will trigger when the picture moves under the cursor. (May impact performance.)
 * @default false
 */
...
'use strict';
    ...
    // ========= Init Routines ========= //
        ...
        $._initBinds = function() {
            ...
            $.disabled = [];        // if includes trigger + picId, disabled, else not. Now for the save stuff (back-compatible)...
        };
        $._initBinds();
    ...
    // ======== Utility (local) ======== //
        ...
        $.mkFunc = function(text) {
            ...
            return new Function('pictureId', 'trigger', text);
        };
        ...
        $._decodeFct = function(body) {
            return isNaN(body) ? $.mkFunc(body) : body;
        };
        ...
        $.savExtract = function(contents) {
            ...
            const I = D.d ? D.r : D;            // back-compatibility
            const r = {};
            for (const t in I) {
                r[t] = {};
                for (const b in I[t]) {
        ...
        $.com = {
            ...
            bindScript: function(o) {
                ...
                if (picId && trigger && script) {
                    return !!$.setBind(picId, trigger, $.mkFunc(script));
                }
                return false;
            },
            /**
