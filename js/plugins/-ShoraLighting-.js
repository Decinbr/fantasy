...
if (Shora.Lighting.PARAMETERS.version.toUpperCase() == 'MV') {
    ...
    Game_Interpreter.prototype.pluginCommand = function(command, args) {
        ...
        if (command) {
            ...
            if (command === 'ambient') {
                $gameLighting.setMapAmbient(Number(args[0]), args[1] || 0);
            } else if (command === 'shadowambient') {
                $gameLighting.setShadowAmbient(Number(args[0]));
            } else if (command === 'topblockambient') {
                $gameLighting.setTopBlockAmbient(Number(args[0]));
...
class GameLighting {
    ...
    // command
    setMapAmbient(color, time) {
        $shoraLayer.lighting.setMapAmbient(color.toHexValue(), time);
    }
