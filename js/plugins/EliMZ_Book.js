...
@plugindesc v4.1.2 - Essential plugin for all Eli plugins.
...
- Change a little bit the way Eli.Utils calculate the screen position of 
an object.
Version 4.1.1 - 09/27/2021
- Fixed a bug that the rgba colors are always with alpha 255.
Version 4.1.0 - 09/23/2021
- Add a new way to handle easing animations using anime.js.
(https://animejs.com/)
- Add a basic collision Library called Bump.js (For future plugins)
(https://github.com/kittykatattack/bump)
- Improved the color manager object that handles color parameters and plugin 
commands in the plugins.
...
Eli.Utils = {
    ...
    getTextSettings(text){
        ...
        return tempWin.textSizeEx(text)
    },
...
Eli.ColorManager = {
    ...
    isRgb(color){
        return color instanceof Array || color.includes(",")
    },

    isHtmlColor(color){
        return color[0] !== "#" && isNaN(color[0])
    },
    ...
    formatRgbToArray(color){
        if(typeof color === "string"){
            color = color.split(",")
        }

        if(color.length === 3) color.push(0)
    ...
    getRgb(color){
        if(this.isHtmlColor(color)){
            ...
            return this.formatRgbToArray(color)
